...
import { callFalLlm, extractJsonFromLlm } from "@/lib/fal/client";
...
function sanitizePlannerOutput(raw: unknown, task: Task, fallback: PlannerOutput): PlannerOutput {
  ...
  const demoIntegrations = (data.demoIntegrations ?? fallback.demoIntegrations)
    .filter(
      (d) =>
        d?.integrationId &&
  ...
  return {
    ...
    calendar: {
      ...
      description: calendar.description?.trim() || fallback.calendar.description,
    ...
    demoIntegrations: demoIntegrations.length > 0 ? demoIntegrations : fallback.demoIntegrations,
  };
}
...
export async function planActionsForTask(
  ...
  const fallback = buildFallbackPlan(task, { defaultEmail: options?.defaultEmail });
  ...
  try {
    const raw = await callFalLlm(buildUserPrompt(task, options?.transcript), buildSystemPrompt());
